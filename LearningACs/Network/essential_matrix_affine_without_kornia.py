...
import torch
from torch import Tensor  # 修复: 添加 Tensor 类型导入
from kornia_moons.feature import *
from kornia.feature import (
    extract_patches_from_pyramid, make_upright, get_laf_scale, 
    rotate_laf, laf_from_center_scale_ori
)  # 修复: LAFAffNetShapeEstimator 依赖此函数
from kornia.geometry.conversions import angle_to_rotation_matrix, rad2deg  # 修复: laf_from_ours, get_laf_orientation 依赖此函数
...
def scale_laf(laf, scale_coef) :
    ...
    if  len(scale_coef.shape) ==1:
        ...
        scale_coef = (mrSize * scale_coef).clone().detach().to(device=device, dtype=torch.float).view(1, N, 1, 1)
